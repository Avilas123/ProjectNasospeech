...
import Speech.phoneme.KeyWordBuilder;
import Speech.phoneme.RightClickEventPhoneme;
/**
 *
...
//            item = new JMenuItem(" Combined ");
            ...
            if (pWave.mousePosX1 != 0 && pWave.mousePosX2 != 0 && pWave.mousePosY1 > 195) {
                try {
                    ...
                    if ((Math.abs(dragStPos - ann_actualStPixPos)) < 15) {
                        ...
                        if (option == JOptionPane.YES_OPTION) {
                            boolean updated = new UpdateAnnotation(pWave.mainFrame.getConn()).resizeUpdate(ann_actualStPos, ann_actualEnPos, changeStarPos, changeEndPos, Hash.getHashValue(StreamConverter.byteTostream(pWave.streamBytes.getCurrent(), pWave.audioInputStream)), pWave.mainFrame.getUserID(), ann_level);
                            if (!updated) {
    ...
    public void waveDelete(int startSample, int endSample) {
        try {
            ...
            if (findAnnotation.isAnnotationAtback()) {
                findAnnotation.changeRemovePostionAnnotation(endMs, (endMs - startMs));
                ...
                pWave.streamBytes.setCurrent(cutW.getresultByteArray());
                //saveFileAsSelected("conf/temp/redo.wav", pWave.streamBytes.getCurrent());
                pWave.setStreamDrawGraph();
                pWave.mousePosX1 = 0;
                pWave.mousePosX2 = 0;
            ...
            if (pWave.mousePosX1 != 0 && pWave.mousePosX2 != 0) {
                ...
                if (pWave.streamBytes.getCurrent() != null && fileName != null) {
                    ...
                    if (findAnnotation.isFileAnnotation()) {
                        int option = javax.swing.JOptionPane.showConfirmDialog(pWave.mainFrame, "Do you want to save annotation to " + fileName, "Save", JOptionPane.YES_NO_OPTION);
                        if (option == JOptionPane.YES_OPTION) {
                            File getFileName = new File(fileName);
                            InsertAnnotation insertAnn = new InsertAnnotation(pWave.mainFrame.getConn());
                            if (!insertAnn.insertSaveASAnnotation(getFileName.getName(), Hash.getHashValue(StreamConverter.byteTostream(pWave.streamBytes.getCurrent(), pWave.audioInputStream)))) {
                                javax.swing.JOptionPane.showMessageDialog(pWave, "Unable to create label file", "Warning", javax.swing.JOptionPane.WARNING_MESSAGE);
                            }
                        }
                    }
