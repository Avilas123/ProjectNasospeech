 ...
 *
 */
public class PlotWavePhoneme extends javax.swing.JPanel implements ActionListener, ControlContext, PlotwaveCommon {
    ...
    private int graphFromScreen = 10, graphVerticalSize = 220, normalPixcel = 60;
    public boolean selectedPlay = false;
    public StreamVariables streamVariable;
    private RightClickEventPhoneme rightClick;
    public ResultKeyWordPanel keyDisplay;
    public KeyWordBuilder keyBuilder;
    public KyeColorHandler keyColorHandler;
    ...
    private boolean lineStatus = false;
     ...
     public PlotWavePhoneme(MainFrame mainFrame, AudioInputStream audioStreamArray, String language, ArrayList processedList) {
        initComponents();
        // Create Objects Area
        ...
        rightClick = new RightClickEventPhoneme(this);
        ...
        try {
            ...
            File audioFile = new File(mainFrame.getUserID() + "phoneme" + ".wav");
            if (audioFile.exists()) {
                audioFile.delete();

            }

            StreamConverter.byteTowavefile(tempCurrent, audioStreamArray, mainFrame.getUserID() + "phoneme" + ".wav");
            audioStreamArray = StreamConverter.byteTostream(tempCurrent, audioStreamArray);
            boolean result = keyBuilder.setSource(mainFrame.getUserID() + "phoneme" + ".wav", language, mainFrame.getUserID() + "phoneme", processedList);
        ...
        keyDisplay = new ResultKeyWordPanel(this);
        keyColorHandler = new KyeColorHandler(this);
        // End Object Area
    ...
    private void initComponents() {
        ...
        jToolBar1.setMinimumSize(new java.awt.Dimension(200, 70));
        jToolBar1.setPreferredSize(new java.awt.Dimension(200, 70));
        ...
        playB.setMaximumSize(new java.awt.Dimension(33, 33));
        ...
        pausB.setMaximumSize(new java.awt.Dimension(33, 33));
        ...
        jButton3.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Speech/Icons/btn_magnificationOne.png"))); // NOI18N
        ...
        samplingPanel.setBorder(javax.swing.BorderFactory.createEmptyBorder(1, 1, 1, 1));
...
//File Load  
    ...
    public void stopSound() {
        try {
            ...
            pausB.setEnabled(false);
            playB.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Speech/Icons/btn_play_icon.png")));
            pausB.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Speech/Icons/btn_pause_icon.jpg")));
            playB.setToolTipText("play");
            pausB.setToolTipText("pause");
        } catch (Exception er) {
            System.err.println(er);
        ...
        try {
            if (file != null && file.isFile()) {
                try {
                    ...
                    Logger.getLogger(PlotWavePhoneme.class.getName()).log(Level.SEVERE, null, ex);
...
//Error Report Method
    ...
    class SamplingGraph extends JPanel implements Runnable {
        ...
        public SamplingGraph() {
            setBackground(new Color(255, 255, 255));//background color for energy contor wave
...
//End Bit calcution
                ...
                //System.out.println("ffff "+lines.size());
                ...
                energyVal = new int[audioDataNormalize.length];
                int temp;
                int shift=0,rep;
                
                for(int i=0;i<audioDataNormalize.length;i++){
                    energyVal[i] = 0;
                }
                
               
                
               PixcelConversion pc = new PixcelConversion();
               int startms =     pc.pixcelToMillisecond(startPaint, frames_per_pixel, (int)format.getFrameRate());
               int frameNo = (int)(startms/160);
               int endms =     pc.pixcelToMillisecond(endPaint, frames_per_pixel, (int)format.getFrameRate());
               int endframe = (int)(endms/160);
                
                ///// added to draw the energy contour  /
                for (int col = frameNo; col < keyColorHandler.getStartPixel().size(); col++) {
                    temp = Integer.parseInt(keyColorHandler.getKeyWordName().get(col).toString());
                    
                    
                    for(rep=0;rep<80;rep++){
                        if(shift+rep < audioDataNormalize.length)
                            if(temp==0)
                               energyVal[shift+rep] = 0;
                            else
                                energyVal[shift+rep] = (int)max_y - 100;
                        else
                            break;
                    }
                    shift = shift+80;
...
//                PixcelConversion pc = new PixcelConversion();
//               int ms =     pc.pixcelToMillisecond(startPaint, frames_per_pixel, (int)format.getFrameRate());
//               int sampNo = 8*ms + 1;
//               int idx1 = sampNo-1;
               ...
               inc2 = 0.02;
               double y_last1=0.0;
               //endPaint = endPaint +startPaint;
               startPaint = 0;
                for (double x1 = startPaint; x1 < endPaint; x1 = x1 + 0.02) {
                    inc2 = inc2 + 0.02;
                    int idx1 = (int) (frames_per_pixel * numChannels * inc2);
                    
                    if (idx1 >= audioDataNormalize.length) {
                        break;
                    }
                    
                   /* if (format.getSampleSizeInBits() == 8) {

                        my_byte = (byte) energyVal[idx];
                    } else {
                        my_byte = (byte) (1400 * energyVal[idx] / 32000);
                    }*/
                    
                    double y_new1 = (double) (h * (90 - energyVal[idx1]) / 210);
                    lines2.add(new Line2D.Double(x1, y_last1, x1, y_new1));
                    y_last1 = y_new1;
                    ...
                    if (audioInputStream != null && format != null) {
                        ...
                        for (int t = startPaint; t < endPaint; t++) {
                            ...
                            if ((t * frames_per_pixel) % format.getFrameRate() < frames_per_pixel) {
                                ...
                                g2.setColor(Color.black);
                                g2.drawString(totalTD, t - 45, h - 22);
                        ...
                        if (iscolorKeyWordAvailable) {
                            ...
                            for (col = 0; col < keyColorHandler.getStartPixel().size(); col++) {
                                ...
                                if (keyColorHandler.getKeyWordName().get(col) != null) {
                                 ...
                                 // System.out.println("Current KeyWord " + currentKeyword + " Stat " + keyStPos + " End " + keyEnPos + " Screen " + samplingGraph.getWidth() + " " + samplingPanel.getWidth() + " but " + startPaint + " " + endPaint);
                                g2.setColor(Color.blue);
                                if ((keyEnPos - keyStPos) > 10) {
                                    g2.setFont(new Font("Serif", Font.PLAIN, 10));
                                    //below lines for display O and 1 in x axies
                                    //g2.drawString(currentKeyword.toLowerCase(), keyStPos + 5, h - 5);
                                    //g2.setColor(Color.red);
                                    //g2.drawLine(keyStPos, h - 15, keyStPos, h);
                                    //g2.drawLine(keyEnPos, h - 15, keyEnPos, h);
                                    //end
                                    g2.setFont(new Font("Serif", Font.PLAIN, 14));
                                    /*g2.setColor(Color.GREEN);
                                    if(currentKeyword.equals("0"))
                                        g2.drawLine(keyStPos, h-105, keyEnPos, h-105);
                                    else
                                        g2.drawLine(keyStPos, h-155, keyEnPos, h-155);
                                    */
                                }
