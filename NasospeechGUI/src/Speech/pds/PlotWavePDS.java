 ...
 *
 */
public class PlotWavePDS extends javax.swing.JPanel implements ActionListener, ControlContext, PlotwaveCommon {
    ...
    public keyColorHandler keyColorHandler1;
    private int graphFromScreen = 10, graphVerticalSize = 200, normalPixcel = 60;
    public boolean selectedPlay = false;
    public StreamVariables streamVariable;
    private RightClickEventPDS rightClick;
    public String[][] annotationPos;
    public PartialPlay partialPlay;
    public String processMethod;
    String fileHashValue;
    public boolean playBacStatus = false;
    private boolean buffStatus = true;
    private boolean lineStatus = false;

    public PlotWavePDS(MainFrame mainFrame, AudioInputStream audioStreamArray, String method, String threshold) {
        initComponents();
        // Create Objects Area

        if (audioStreamArray == null || method == null) {
            return;
        }
        ...
        partialPlay = new PartialPlay();
        streamVariable = new StreamVariables();
        rightClick = new RightClickEventPDS(this);
        keyBuilder = new KeyWordBuilder();
        keyColorHandler1 = new keyColorHandler(this);
        ...
        try {
            ...
            File audioPdsFile = new File("PDS/pds.wav");
            if (audioPdsFile.exists()) {
                audioPdsFile.delete();
            ...
            fileHashValue = Hash.getHashValue(audioStreamArray);
            audioStreamArray = StreamConverter.byteTostream(tempCurrent, audioStreamArray);
            ArrayList annotationArray = null;
            try {
                GetAnnotation getAnnotation = new GetAnnotation(mainFrame.getConn());
                annotationArray = getAnnotation.getAllTimes(fileHashValue);
            } catch (Exception er) {
                System.err.println(er.getMessage());
            }
            boolean result = keyBuilder.setSource(mainFrame.getUserID() + ".wav", method, mainFrame.getUserID(), annotationArray, threshold);
            if (method.startsWith("Enhance") || method.startsWith("Combined") || method.startsWith("Combined")) {
                try {
        ...
        wavePanelLayout.setHorizontalGroup(
            ...
            .addGroup(wavePanelLayout.createSequentialGroup()
                .addGroup(wavePanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(timeLabel, javax.swing.GroupLayout.PREFERRED_SIZE, 457, javax.swing.GroupLayout.PREFERRED_SIZE))
                .addContainerGap())
        );
    ...
    public void stopSound() {
        try {
            ...
            Logger.getLogger(PlotWavePDS.class.getName()).log(Level.SEVERE, null, er);
        }
    }
    ...
    private void wavePanelMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_wavePanelMousePressed
        // TODO add your handling code here:
    }//GEN-LAST:event_wavePanelMousePressed
    ...
    javax.swing.JToggleButton pausB;
                ...
                while (thread != null) {
                    try {
                        ...
                        while (numBytesRemaining > 0) {
                            // System.out.println("Than - "+buffStatus+" "+lineStatus);
                            Thread.sleep(5);
                            if (buffStatus) {
    ...
    class SamplingGraph extends JPanel implements Runnable {
        ...
        public void createWaveForm(byte[] audiobyte) {
            ...
            try {
                ...
                if (keyColorHandler1.sourceValidation(frames_per_pixel)) {
                    if (keyColorHandler1.isKeywordFind()) {
                ...
                for (double x = startPaint; x < endPaint && audioDataNormalize != null; x = x + 0.02) {
                    ...
                    if (format.getSampleSizeInBits() == 8) {
                        ...
                        my_byte = (byte) (1500 * audioDataNormalize[idx] / 32000);
                    }
        ...
        public void paint(Graphics g) {
            try {
                ...
                if (errStr != null) {
                    ...
                    g2.setFont(font12);
                    timeLabel.setText("Length: " + String.valueOf(seconds));
                } else {
                    g2.setColor(Color.black);
                    g2.setFont(font12);
                    // timeLabel.setText("File : " + fileName + "        Length : " + String.valueOf(duration) + "        Position : " + String.valueOf(seconds));

                    int startPaint = jScrollPane1.getHorizontalScrollBar().getValue();
                    /*
                     * if (playback.thread != null) { if (mousePosX1 != 0 &&
                     * mousePosX2 != 0) {
                     *
                     * int minValue = (int) ((mousePosX1 - mousePosX2) < 0 ?
                     * mousePosX1 : mousePosX2) + startPaint;
                     *
                     * double intial_second = ((minValue * frames_per_pixel) /
                     * audioInputStream.getFormat().getFrameRate()) + seconds;
                     *
                     * timeLabel.setText("File : " + fileName + " Length : " +
                     * String.format("%.1f", duration) + " Position : " +
                     * String.format("%.1f", intial_second));
                     *
                     * } else if (mousePosX1 != 0 && mousePosX2 == 0) {
                     *
                     * double intial_second = ((mousePosX1 * frames_per_pixel) /
                     * audioInputStream.getFormat().getFrameRate()) + seconds;
                     *
                     * timeLabel.setText("File : " + fileName + " Length : " +
                     * String.format("%.1f", duration) + " Position : " +
                     * String.format("%.1f", intial_second)); } else {
                     * timeLabel.setText("File : " + fileName + " Length : " +
                     * String.format("%.1f", duration) + " Position : " +
                     * String.format("%.1f", seconds)); } } else {
                     * timeLabel.setText("File : " + fileName + " Length : " +
                     * String.format("%.1f", duration) + " Position : " +
                    ...
                    if (audioInputStream != null && format != null) {
                        ...
                        Color mouseDraggedbg;
                        if (partialPlay.thread != null) {
                            mouseDraggedbg = new Color(255, 255, 255);
                        } else {
                            mouseDraggedbg = new Color(255, 255, 153);
                        }
                        ...
                        if (annotationPos != null) {
                            if (annotationPos.length > 0) {
                                ...
                                for (int ann = 0; ann < annotationPos.length; ann++) {
                                    ...
                                    g2.setStroke(stroke);
                                    try {
                                        g2.drawRect((int) anStartpos, 30, (int) anEndpos - anStartpos, h - INFOPAD - 60);
                                    } catch (Exception er) {
                                    }
                        ...
                        if (iscolorKeyWordAvailable) {
                            keyColorHandler1.setColorIndex(startPaint, endPaint);
                        }
                            ...
                            try {
                                ...
                                if (((Line2D) lines.get(i)).getY2() != 0 && ((Line2D) lines.get(i)).getY1() != 0) {
                                    ...
                                    if (iscolorKeyWordAvailable) {
                                        ...
                                        int screenXpos = (int) (i / 10), keyStPos = 0, keyEnPos = 0;
                                        String currentKeyword = "";
                                        screenXpos = startPaint + screenXpos;
                                        if (i % 10 == 0) {
                                            drawColor = false;
                                            for (col = 0; col < 0; col++) {
                                                keyStPos = Integer.parseInt((keyColorHandler1.getStartPixel().get(col)).toString());
                                                keyEnPos = Integer.parseInt((keyColorHandler1.getEndPixel().get(col)).toString());
...
//End loop of drawing signals and Time lines
                        ...
                        //Draw rectangle for silence Regions

                        PixcelConversion pixcelConvert = new PixcelConversion();
                        int startDrawRect = 0, endDrawRect = 0;
                        if (iscolorKeyWordAvailable) {
                            for (int i = 0; i < keyBuilder.getStartTimeList().size(); i++) {
                                try {
                                    startDrawRect = pixcelConvert.milliSecondToPixcel(keyBuilder.getStartTimeList().get(i).toString(), frames_per_pixel, (int) audioInputStream.getFormat().getFrameRate());
                                    endDrawRect = pixcelConvert.milliSecondToPixcel(keyBuilder.getEndTimeList().get(i).toString(), frames_per_pixel, (int) audioInputStream.getFormat().getFrameRate());
                                    g2.setColor(new Color(130, 130, 130));
                                    g2.drawRect(startDrawRect, 20, (endDrawRect - startDrawRect), h - 60);
                                    if (processMethod.startsWith("Remove noise")) {
                                        if ((endDrawRect - startDrawRect) > 60) {
                                            g2.setColor(new Color(0, 0, 0));
                                            g2.setFont(new Font("Courier New", Font.BOLD, 11));
                                            g2.drawString("Noise", startDrawRect + 10, 35);
                                        } else if ((endDrawRect - startDrawRect) > 30) {
                                            g2.setColor(new Color(0, 0, 0));
                                            g2.setFont(new Font("Courier New", Font.BOLD, 11));
                                            g2.drawString("Noi", startDrawRect + 8, 35);
                                        }
                                    } else {
                                        if ((endDrawRect - startDrawRect) > 60) {
                                            g2.setColor(new Color(0, 0, 0));
                                            g2.setFont(new Font("Courier New", Font.BOLD, 11));
                                            g2.drawString("Silence", startDrawRect + 10, 35);
                                        } else if ((endDrawRect - startDrawRect) > 30) {
                                            g2.setColor(new Color(0, 0, 0));
                                            g2.setFont(new Font("Courier New", Font.BOLD, 11));
                                            g2.drawString("Sil", startDrawRect + 8, 35);
                                        }
                                    }
                                } catch (Exception er) {
                                    System.err.println(er.getMessage());
                        ...
                        if (seconds != 0 && !selectedPlay) {
                            ...
                            if ((((int) loc % (jScrollPane1.getWidth())) < 50) && loc != 0 && loc != 0 && ((loc + jScrollPane1.getWidth()) < (samplingGraph.getWidth() - 10))) {
...
//*********************************************************************************************************
    ...
    public class PartialPlay implements Runnable {
        ...
        public void start() {
            ...
            this.thread.setName("PartialPlay");
            this.thread.start();
        }
        ...
        public void run() {
            try {
                ...
                if (this.thread != null) {
                    if (keyBuilder.getStartTimeList() != null) {
                        for (int i = 0; i < keyBuilder.getStartTimeList().size(); i++) {
                            if (this.thread == null) {
                                mousePosX1 = 0;
                                break;
                            ...
                            try {
                                ...
                                try {
                                    startRemove = pixcelConvert.milliSecondToPixcel(keyBuilder.getStartTimeList().get(i).toString(), frames_per_pixel, (int) audioInputStream.getFormat().getFrameRate());
                                    endRemove = pixcelConvert.milliSecondToPixcel(keyBuilder.getEndTimeList().get(i).toString(), frames_per_pixel, (int) audioInputStream.getFormat().getFrameRate());
                                ...
                                mousePosX1 = startMilliTime;
                                mousePosX2 = endMilliTime;
                                if (((int) (mousePosX1)) == 0) {
                                ...
                                // javax.swing.JOptionPane.showMessageDialog(null,"Pix is "+mousePosX1+","+mousePosX2);
                                if (rightClick.sourceAvailValidation()) {
                                    System.out.println("Test Play " + mousePosX1 + "," + mousePosX2);
                                    rightClick.playSoundAll();
                                }

                                //jScrollPane1.getViewport().setViewPosition(new java.awt.Point((int) startMilliTime - 50, 0));
                                while (playback.thread != null) {
                                    Thread.sleep(100);
                                }

                                if ((i + 1) == keyBuilder.getStartTimeList().size()) {
                                    mousePosX1 = endRemove;
                                    mousePosX2 = samplingGraph.getSize().width - 10;
                                    if (rightClick.sourceAvailValidation()) {
                                        rightClick.playSoundAll();
                                    }
                ...
                samplingGraph.repaint();
                jScrollPane1.getViewport().setViewPosition(new java.awt.Point((int) 0, 0));
            } catch (Exception er) {
                System.err.println(er);
            }
        }
