...
public class FXMLDocumentController extends Application {
    ...
    private LineChart<?, ?> lineChartForRecord;
     ...
     @FXML
    private MenuItem Select_Hypernasality;
       @FXML
...
//counterap.setStyle("-fx-border-color: black");
  ...
  TP.getTabs().addAll(tab1);
    ...
    RamerDouglasPeuckerFilter rdpf = new RamerDouglasPeuckerFilter(0.1);
       ...
       // double duration=numsamples/ factor;
        ...
        nfx.tempplot(classStage,filename,samples, numSamples, tab1, TP, wavepane, factor, frames_per_pixel);
 ...
 @FXML
    void hypernalasityclick(ActionEvent event) throws UnsupportedAudioFileException, IOException {
             Tab tab = new Tab();
            tab.setText("Hypernasality  ");
            String filename = this.getfilename(); 
            File sendfile = new File(filename);//fileName);
            String newFilename = null;
            newFilename = SampleRateConversion(sendfile, (float) 8000.0);
            System.out.println("The converted file is "+ newFilename);
            TP.getTabs().add(tab);
            ...
            TP.setTabClosingPolicy(TabPane.TabClosingPolicy.SELECTED_TAB);
            double probability = nfx.Hypernasality(newFilename);
   ...
   @FXML
    void Select_Check_hypernalasityclick(ActionEvent event) throws UnsupportedAudioFileException, IOException, LineUnavailableException {
        ...
        Tab tab = new Tab();
         ...
         //  String filename = this.getfilename(); //fileName);
            ...
            TP.getSelectionModel().select(tab);
            TP.setTabClosingPolicy(TabPane.TabClosingPolicy.SELECTED_TAB);
            double probability = nfx.Hypernasality(Select_Check_hypernalasityclick);
    ...
    tab.setContent(t);
     File file,file1;
    file=new File("C:\\Users\\Naso\\Documents\\NetBeansProjects\\MouseEvent\\cexe\\Hypernasal.wav");
    file1=new File("C:\\Users\\Naso\\Documents\\NetBeansProjects\\MouseEvent\\cexe\\mfcc_output_13dim.txt");
    if(file.delete()){
     ...
     //   data.add(dd);
      // } 
          XYChart.Series series = new XYChart.Series();
           ...
           lineChartForRecord.setAnimated(false);
         ...
         //  duration=plot.getduration();
          ...
          //     double[] samples = plot.readAudioData(audiobytes, format);
                   ...
                   byte audioBytes[] = capOut1.toByteArray();
                    ...
                    range=Checknumsamples- lastVal;

                    
       // System.out.println("Checknumsamples"+Checknumsamples+"lastval"+lastVal+"range"+range);
                  ...
                  if( range<3000)
                {
                    ...
                    {  RamerDouglasPeuckerFilter rdpf1 = new RamerDouglasPeuckerFilter(0.1);
