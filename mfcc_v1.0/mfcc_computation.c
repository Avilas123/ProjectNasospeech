...
FILE *myfile1;
FILE *myfile2,*MATRIX_ARTI,*test_NEW, *MATRIX1,*MATRIX2, *transpose_1,*transpose_2,*transpose_3, *target_A, *mean_in_KAKA,*mean_PAPA,*mean_in_TATA,*mean_in_tata,*malo_1T,*twoDDCT_IN,*covar_in, *malo_2I,*DFT_IN,*check_NW1,*check_NW2,*check_NW3;
...
{   
    ...
    MATRIX2 = fopen("C2.txt","r");
    myfile2 = fopen("PATCHING.txt","w");    
    transpose_1 = fopen("T_1.txt","w");
    transpose_2 = fopen("T_2.txt","w");
   ...
   // mean_in_tata = fopen("mean_t.txt", "r");
    ...
    test_NEW= fopen("test_NEW.txt","w");
    
    MATRIX_ARTI=fopen("C_matrix_articulation.txt", "r");
    //float DFT_transpose[250][950];
    
    int k,l,m,i,j,p,q,r,a,b,c,d,e,f,g,h,p1,r1,x,y,x1,y1,f1,f2,h1,g1;
    int rslt1, rslt2;
    ...
    float mult_NEW[30][5];
    float mult_new[70][70],covar_mult[36][13],covar_mult1[1][1];
    char cover_mult1_char[1][1];
    float malo1_transpose[36][13];
    float A_tar[60][60];
    //float mean[27][1];
    float covariance[28][28],substract_transpose[27][1];
    float TWOddct[27][1];
    float sum=0,sum1=0,sum2=0,sum3=0,sum4=0;
    float malo_1[13][36]; /*malo_2[27][27],malo_3[27][27]*/
    //float mean_new[27][1];
    float new_mult_arti[70][70];
    ...
    {
         {
            ...
            //printf("ARRAY G IS \t%f\n",arr1[k]);
             ...
             float p = (float)strtod(mouse,NULL);
             //printf("\t\t\t\t 'P' IS \t %f \n",p);                              ////////////////////////////////convert char to float 
             //float q = (float)strtod(mouse,NULL);
             //float p=1872;
             float q=4689;
    ...
    for(int i=0;i<13;i++)
    {
        for(int j=0;j<22;j++)
        {
           fscanf( MATRIX_ARTI, "%f", &matrix1[i][j]);                               //////// (C1.txt) is transposed////////
           //fscanf( MATRIX2, "%f", &matrix2[i][j]);
           //printf("matrix1@ is \t%f\t   ", matrix1[i][j]);                         ////////MODIFIED NOW 9/4/2018////////////
    ...
    for(int p=(rslt1-1),a=0;p<(rslt2+1),a<36;p++,a++){
        for(int q=0;q<13;q++){
            for(int r=0;r<NUM_OF_FILTER;r++){
                
               sum= sum+ (NEW_patching_1[p][r]*transpose_C1[r][q]);
               //printf("RESULT NOW IS----->> %f\t\t\t ",NEW_patching_1[p][r]);         //////////CHANGE upto here 9/04/2018///////// 
    ...
    {
        for(int j=0;j<36;j++){
            new_mult_arti[i][j]=multiply1[j][i];
            //printf("The result is  %f\t\t\t ",new_mult_arti[i][j]);                     /////////////CHANGED 9/04/2018 ////////////
            fprintf(test_NEW, "%f\t",new_mult_arti[i][j] );                             
        }
        fprintf(test_NEW, "\r\n");                                                      /////////// NEW "C" matrix /////////////// 
    }
...
//FILE *covar_in;
FILE *myfile2,*mean_PAPA,*mean_KAKA,*mean_tata,*mean_TATA,*MEAN_2,*COVAR_2,*cov_KAKA,*cov_PAPA,*cov_tata,*cov_TATA;
void automatic_selection(float mahal_out[],char *x,int totalFrames,char *mouse)/////mouseposition is updated
{   
    ...
    MEAN_2=fopen("new_MEAN.txt","r");
    COVAR_2=fopen("new_VAR.txt","r");
...
//             printf(" k is printed");
         ...
         case 'T':
            ...
            case 'h':
             ...
             for(p=0;p<13;p++){
                    for(q=0;q<1;q++){
                          fscanf(MEAN_2," %f", &mean[p][q]);
                           //printf("mean is %f\n",mean[p][q]); 
                    }
             }
             for(int i=0;i<13;i++){
                     for(int j=0;j<13;j++){
                            fscanf(COVAR_2," %f", &covar[i][j]);                              /////READ the covariance matrix from txt file.
                            //printf("covariance matrix is\t %f\n", covar[i][j]);
        }
    }
