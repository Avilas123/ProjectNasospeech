...
////do            nfx.paste(this.audioInputStream,tab1,TP,wavepane1);
      ...
      private Duration time = Duration.ZERO;
            @FXML
             void record(ActionEvent event) throws LineUnavailableException,UnsupportedAudioFileException, IOException, Exception
             {  
                 AudioFormat format=new AudioFormat(AudioFormat.Encoding.PCM_SIGNED,8000,16,2,4,8000,false);
                 {
                     DataLine.Info info=new DataLine.Info(TargetDataLine.class, format);
                     TargetDataLine targetline=(TargetDataLine)AudioSystem.getLine((javax.sound.sampled.Line.Info) info);
                     targetline.open();
                     ...
                     targetline.start();
                     byte[] data=new byte[targetline.getBufferSize()/8];
                     int readBytes;
                     while(true){
