      ...
      // } 
        ...
        //  XYChart.Data<Double,Double> dd;
          ...
          // data.add(dd);
           ...
           lineChartForRecord.getXAxis().setAutoRanging(true);
           lineChartForRecord.getYAxis().setAutoRanging(false);
                  ...
                  if( range<5000)
                {
                  ...
                  //   numsamples=fpoints.length;
                    System.out.println("start"+start);
                     for(int i=start;i<Lastvalue;i++)
                    { 
                        int finalI=i;
                        if(series.getData().size()>2000){
                        //Platform.runLater(()->data.remove(0));   
                        Platform.runLater(()->series.getData().remove(0)); 
                        } 
                         //  data.remove(0);
                            Platform.runLater(()->
                      //  series.getData().add(new XYChart.Data<>(finalI/freq,samples[finalI])));
                        
//data.add(new// XYChart.Data<>((double)finalI/freq,samples[finalI]))
                         //dd= new XYChart.Data<>((double)finalI/freq,samples[finalI])
                               
                        data.add(new XYChart.Data<>((double)finalI/freq,fpoints[finalI])));
           ...
           //         Platform.runLater(()-> series.getData().add(data)); 
                    // if(series.getData().size()>5000)
                     //  series.getData().remove(0,range);
