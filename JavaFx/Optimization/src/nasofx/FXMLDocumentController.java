...
//counterap.setStyle("-fx-border-color: black");
 ...
 //nfx.setTab(tab1);
  ...
  //  RamerDouglasPeuckerFilter rdpf = new RamerDouglasPeuckerFilter(0.01);
      ...
      //   nfx.createAudioWaveForm(classStage, filename, samples, numsamples, tab1, TP, wavepane, factor, duration);
       this.createAudioWaveForm(samples, numsamples, wavepane);
    }
 /////////////////////conversion/////////////
  @FXML
...
public void createAudioWaveForm(double[] samples,int numsamples,ScrollPane wavepane ){
             Pane pane=new Pane();
             wavepane.setContent(pane); 
             Plotwave plot=new Plotwave();
         int numChannels= plot.format.getChannels();
                 lines.removeAllElements();
                  double inc = 0.02;
                  byte my_byte = 0;
                  double y_last = 0;
                  int h=(int)wavepane.getHeight();
                // Calculate Screen Pixels
             int  startPaint=0;
             int  endPaint=1140;
             frames_per_pixel=7;
                for (double x = startPaint; x < endPaint && samples != null; x = x + 0.02) 
                {   inc = inc + 0.02;
                    ...
                    if (format.getSampleSizeInBits() == 8) {
                        ...
                        my_byte = (byte) samples[idx];
                    } else {
                        my_byte = (byte) (2100 * samples[idx] / 32000);
                    }
